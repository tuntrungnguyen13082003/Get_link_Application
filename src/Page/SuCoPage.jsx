...
// Import ảnh mẫu
import anhminhhoa1 from '../assets/Su_co/Ref_1.jpg';
import anhminhhoa2 from '../assets/Su_co/Ref_2.jpg';
import anhminhhoa3 from '../assets/Su_co/Ref_3.jpg';
...
const App = () => {
  ...
  // Trạng thái kiểm tra mã ban đầu
  const [isCheckingCode, setIsCheckingCode] = useState(true); 
  const [sessionStatus, setSessionStatus] = useState("checking"); // checking | active | used | invalid

  // 1. BIẾN ĐỂ LƯU MÃ REAL (QUAN TRỌNG)
  const [realCode, setRealCode] = useState("");

  const queryParams = new URLSearchParams(window.location.search);
  const fakeTokenFromUrl = queryParams.get("code");

  // --- CẤU HÌNH LINK GOOGLE SCRIPT ---
  ...
  // --- 1. TỰ ĐỘNG KIỂM TRA MÃ KHI VỪA VÀO WEB ---
  useEffect(() => {
    const checkTokenStatus = async () => {
      ...
      try {
        const response = await fetch(GOOGLE_SCRIPT_URL, {
          method: "POST",
          body: JSON.stringify({
             action: "check_status", // Gọi hành động kiểm tra mới thêm
        ...
        if (data.realCode) {
            console.log("Đã lấy được mã Real:", data.realCode);
            setRealCode(data.realCode); 
        }

      } catch (error) {
        console.error("Lỗi kiểm tra mã:", error);
        setSessionStatus("error"); // Lỗi mạng
      } finally {
        setIsCheckingCode(false);
      }
    };
  ...
  const handleImageCapture = (e, questionId) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        setUserImages(prev => ({ ...prev, [questionId]: event.target.result }));
      };
...
const uploadReport = async () => {
    ...
    try {
      // 1. XỬ LÝ THỜI GIAN (YYYYMMDD)
      const now = new Date();
      
      const yyyy = now.getFullYear(); // Lấy đủ 4 số: 2025
      const mm = String(now.getMonth() + 1).padStart(2, '0');
      const dd = String(now.getDate()).padStart(2, '0');
      const datePrefix = `${yyyy}${mm}${dd}`; // Kết quả: 20251219
      
      // 2. XỬ LÝ MÃ CODE
      const finalCode = (realCode || "Unknown").trim(); 
      
      // 3. TẠO TÊN FOLDER VÀ TÊN FILE ZIP
      const folderName = `ProcessProblem_${finalCode}`; 
      const zipFileName = `${datePrefix}_ProcessProblem_${finalCode}.zip`;
      
      // 4. NÉN FILE (Đoạn dưới này giữ nguyên)
      const zip = new JSZip();
      ...
      const response = await fetch(GOOGLE_SCRIPT_URL, {
        ...
        body: JSON.stringify({
            ...
            fileData: content,
            token: finalCode,     
            sheet_name: "SU_CO" // Nhớ kiểm tra lại tên sheet
        })
      });
